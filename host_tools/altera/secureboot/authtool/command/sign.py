...
def do_sign(args):
    ...
    if is_pheader_image(input_image):
        ...
        if len(embedded_keydata) > AUTHHEADER_IMAGE_ALIGN:
    ...
    if args.rootkey_type == 'fuse':
        ...
        pubkey, root_key_algo = keypair_extract_raw_pubkey(open(root_key, 'rb'))
    ...
    for key_num, key in enumerate(subroot_keys):
        ...
        pubkey, algo = keypair_extract_raw_pubkey(open(key, 'rb'))
    ...
    checksum_pad_length = bytes_to_alignment(header.size() + len(embedded_keydata) + 4, AUTHHEADER_IMAGE_ALIGN)
    ...
    extra_pad_length = bytes_to_alignment(auth_image_length, AUTHHEADER_IMAGE_ALIGN)
    ...
    # Sign the full image.
    image_data = bytearray('').join([header.to_bytes(),
                                     embedded_keydata,
                                     image,
                                     extra_pad])
    image_signature = bytearray('')
