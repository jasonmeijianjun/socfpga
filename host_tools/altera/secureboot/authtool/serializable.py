...
class Serializable(object):
    ...
    def notify_field_update(self, field_name):
        for key in self._update_hooks.iterkeys():
...
class Padding(Serializable):
    ...
    def to_bytes(self):
        return bytearray().join(chr(self.fill) for _ in range(self.size()))
...
class FieldWrapper(object):
    ...
    def __set__(self, instance, new_value):
        ...
        if isinstance(new_value, self._field_type):
            ...
            bogus_items = filter(lambda i: not isinstance(i, old_list._struct_type), new_value)
            if not bogus_items:
...
#                print "{}0x{:08X} 0x{:08X}: {:12} Padding({})".format("  " * depth, offset, new_instance.get_field_offset(new_field_inst), name, size)
    ...
    def get_field_offset(self, field):
        fields_before = itertools.takewhile(lambda (n, f): field is not f,
                                            self.each_field())
        return sum(f.size() for n, f in fields_before)
