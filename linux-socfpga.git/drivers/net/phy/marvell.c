 ...
 */
static int phy_update_link(struct phy_device *phydev)
{
	int status;
	int err;
	int temp;
	err = phy_write(phydev, MII_MARVELL_PHY_PAGE, 1);
	if (err < 0)
		return err;
	temp = phy_read(phydev, 17); //read phy status
//	printk("page 1 register 0x%x\n",temp);
        temp &= (1 << 10 );
	err = phy_write(phydev, MII_MARVELL_PHY_PAGE, 0);
	if (err < 0)
		return err;
 ...
 *
 */
static int phy_loopback(struct phy_device *phydev)
{
	int status;
	int err;
	int temp;
	temp = phy_read(phydev, 0); //read phy status
	err = phy_write(phydev, 0, temp | 1 << 14); //set loop back
	if ( err < 0 ) {
		printk("Error set loop ");
		return err;
	}
...
#endif 	
	if(old_value != phydev->link) {
		old_value = phydev->link;
		printk("phydev->link is %d\n",phydev->link);
	}
...
static struct phy_driver marvell_drivers[] = {
	...
	},
	{
		...
		.features = PHY_GBIT_FEATURES | SUPPORTED_Pause |SUPPORTED_FIBRE,
		.flags = PHY_HAS_INTERRUPT,
		.config_aneg = &m88e1510_config_aneg,
		.read_status = &marvell_read_status,
		.ack_interrupt = &marvell_ack_interrupt,
