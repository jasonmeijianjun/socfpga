...
{  
	...
	char *stop;
	  ...
	  PHY_BASE = strtoll(argv[1], &stop, 16);
	printf("ori---0x%llx---\n", PHY_BASE);
//	  PHY_BASE += 0x100000000;
...
//	printf("after 0x%llx\n", PHY_BASE);
		  ...
		  fd = open("/dev/mem", O_RDWR);  
			...
			for (i = 0;i < SIZE; i+=0x10)  {  
					printf("0x%lx: %8x %8x %8x %8x\n", PHY_BASE+i, *(volatile int*)(map_base+i), 
						*(volatile int*)(map_base+i+4), *(volatile int*)(map_base+i+8), *(volatile int*)(map_base+i+0xc));  
